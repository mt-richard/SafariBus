    ...
    return (  
        <nav onClick={() =>setMenu(!menu)} className="w-100 px-5 md:px-10 py-5 md:flex justify-between">
            ...
            <div className={` w-100  ${open ? 'bg-white opacity-95 absolute mt-10 responce':' '}`}>
                    ...
                    <ul className={`md:flex justify-between absolute left-0  inks md:flex md:items-center md:px-12 md:pb-0 pb-12 absolute md:static md:z-auto z-[-1] left-0  w-full md:w-auto ${open ? 'top-20 opacity-100':'top-[-490px] opacity-100 '}`}>
                        { links.map((e) =>(
